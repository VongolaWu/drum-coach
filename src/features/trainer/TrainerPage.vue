...
import { clonePracticeMeasure } from '../../domain/score/model.js';
import {
  buildLoopedMeasureTimeline,
  findMeasureAtSequenceOffset,
  getLoopedDurationSec,
  getSequenceDurationSec
} from '../../lib/rhythm.js';
...
const currentLoopMeasureIndex = ref(0);
const currentBeatWithinMeasure = ref(0);
...
const sequenceDurationSec = computed(() => getSequenceDurationSec(state.measures.value, state.bpm.value));
const warmupTimeline = computed(() => buildLoopedMeasureTimeline(state.measures.value, state.bpm.value, state.warmupMeasures.value));
const recordingStartMeasureOffset = computed(() =>
  state.measures.value.length ? state.warmupMeasures.value % state.measures.value.length : 0
);
const recordingTimeline = computed(() =>
  buildLoopedMeasureTimeline(
    state.measures.value,
    state.bpm.value,
    state.recordingMeasures.value,
    recordingStartMeasureOffset.value
  )
);
const warmupDurationSec = computed(() => getLoopedDurationSec(state.measures.value, state.bpm.value, state.warmupMeasures.value));
...
const expectedNoteCount = computed(() => {
  const totalSequenceSec = sequenceDurationSec.value;
...
function getPlayheadState(time) {
  ...
  const totalSequenceSec = sequenceDurationSec.value;
  if (!totalSequenceSec) return null;
  ...
  const activeMeasure = findMeasureAtSequenceOffset(state.measures.value, state.bpm.value, localOffset);
  if (!activeMeasure) return null;

  const timeWithinMeasure = localOffset - activeMeasure.measureStartSec;
  const measureRatio = activeMeasure.durationSec ? timeWithinMeasure / activeMeasure.durationSec : 0;

  return {
    measureIndex: activeMeasure.measureIndex,
    timeWithinMeasure,
    x:
      state.geometry.startX +
      measureRatio * (state.geometry.endX - state.geometry.startX)
  };
}

function playQuarterMetronomeClick(isMeasureStart, time) {
...
function buildAnalyzedMeasures() {
  return recordingTimeline.value.map((timelineMeasure, index) => ({
    ...clonePracticeMeasure(timelineMeasure.sourceMeasure),
    id: `analysis-${index + 1}`
  }));
}

function getExpectedNotesInWindow(windowStartTime, windowEndTime) {
  const sequenceDuration = sequenceDurationSec.value;
  if (!sequenceDuration || !state.flatSequenceNotes.value.length) return [];
  const timeline = recordingTimeline.value;
  ...
  for (let cycleIndex = firstCycleIndex; cycleIndex <= lastCycleIndex; cycleIndex += 1) {
    state.flatSequenceNotes.value.forEach((note) => {
      ...
      const relativeTime = absoluteTime - windowStartTime;
      const recordedMeasureIndex = timeline.findIndex(
        (timelineMeasure) => relativeTime >= timelineMeasure.startSec && relativeTime < timelineMeasure.endSec
      );
      if (recordedMeasureIndex < 0) return;
...
function resetSession() {
  ...
  currentLoopMeasureIndex.value = 0;
  currentBeatWithinMeasure.value = 0;
  state.clearVisualStates();
  state.userHitRecords.value = [];
...
function audioSchedulerLoop() {
  ...
  const loopMeasures = state.measures.value;

  while (loopMeasures.length && nextNoteTime.value < audioCtx.value.currentTime + 0.1) {
    playQuarterMetronomeClick(currentBeatWithinMeasure.value === 0, nextNoteTime.value);
    nextNoteTime.value += quarterDuration;
    currentBeatWithinMeasure.value += 1;

    const currentMeasure = loopMeasures[currentLoopMeasureIndex.value];
    const beatLimit = currentMeasure?.timeSignature?.beats ?? currentMeasure?.beats?.length ?? 4;
    if (currentBeatWithinMeasure.value >= beatLimit) {
      currentBeatWithinMeasure.value = 0;
      currentLoopMeasureIndex.value = (currentLoopMeasureIndex.value + 1) % loopMeasures.length;
    }
  }
...
function renderLoop() {
  ...
  const totalSequence = sequenceDurationSec.value;
  if (!totalSequence) return;
  ...
  if (elapsed < warmupEndElapsed) {
    ...
    const remainingMeasures = warmupTimeline.value.filter((measure) => elapsed < measure.endSec).length;
    realtimeFeedback.value = `热身中，还剩 ${remainingMeasures} 小节开始录音。`;
...
async function toggleRun() {
  ...
  try {
    ...
    recordingStartTime.value = sequenceStartAudioTime.value + warmupDurationSec.value;
    recordingEndTime.value = recordingStartTime.value + recordingDurationSec.value;
  ...
  <main class="page">
    <section v-if="currentStep === 'setup'" class="step-shell">
      <ControlPanel
        ...
        @update-beat-count="state.updateMeasureBeatCount"
